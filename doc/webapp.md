...
```bash
curl "http://127.0.0.1:5000/api/week?city=%E6%9D%B1%E4%BA%AC&days=7"
```

2日目より先は、予測した天気を入力に使い直す**再帰予測**です。誤差が積み重なるため、
`interval` の幅が日を追うごとに広がります。3日目以降は当てにしすぎないでください
（くわしい仕組みと限界は [doc/forecast.md](forecast.md) 第7章）。
